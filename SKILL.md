...
When status is `"triggered"`, the plugin stores your message and data on the intent and wakes the target agent via `api.runtime.system.enqueueSystemEvent` + `runHeartbeatOnce`. The target agent sees the trigger in its ACTION NEEDED block on its next turn.
